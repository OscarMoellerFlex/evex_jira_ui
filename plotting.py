...
def create_resolution_time_charts(df):
    ...
    for group_col, group_label in [
        ...
        if group_col:
            ...
            values = completed.get(group_col, pd.Series(index=completed.index, dtype='object'))
            completed[group_col] = values.astype('string').fillna('').str.strip().replace('', 'Unbekannt')
            group_cols.append(group_col)

        result = completed.groupby(group_cols, observed=False)['key'].count().reset_index()
        ...
        fig = px.bar(
            ...
            labels={bin_col: 'Bearbeitungszeit in Stunden', 'key': 'Anzahl Fertige Tickets',
...
def create_toggle_chart(
    ...
    # 1. Data Preparation
    # Group and count
    result = df[[x_col, group_col, count_col]].groupby([x_col, group_col]).count().reset_index()
    ...
    if sort_x_by_total:
        ...
        total_counts_index = result.groupby(x_col)[count_col].sum().sort_values(ascending=False).index
    else:
