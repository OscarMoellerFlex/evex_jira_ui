...
def enrich_jira_time_metrics(
    ...
    subdiv: str | None = None,   # e.g. "BE", "BY", "NW", ...; None = federal holidays only
    ...
    out["time_to_resolution_h"] = (
        (out["resolved_at"] - out[created_col]).dt.total_seconds() / 3600.0
    )

    # ---------- calendar days (0 if same calendar day; only for done) ----------
    out["time_to_resolution_calendar_days"] = (
        (out["resolved_at"].dt.normalize() - out[created_col].dt.normalize()).dt.days
    )
    out.loc[~out["is_done"], "time_to_resolution_calendar_days"] = np.nan

    out["resolution"] = np.where(out["is_done"],
    ...
    holiday_dates = np.array([np.datetime64(d) for d in de.keys()], dtype="datetime64[D]")  # for np.busday_count
    ...
    out["time_to_resolution_biz_hours"] = np.where(
        out["is_done"],
        out.apply(lambda r: business_seconds_between(r[created_col], r["resolved_at"]) / 3600.0, axis=1),
        np.nan
    )
    ...
    if updated_col in out.columns:
        updated_days = out[updated_col].dt.tz_localize(None).to_numpy(dtype="datetime64[D]")
    ...
    resolved_days = out["resolved_at"].dt.tz_localize(None).to_numpy(dtype="datetime64[D]")
    ...
    try:
        ...
        for l in links:
            if "outwardIssue" in l:
                clone_key = l["outwardIssue"]["key"]
                clone_type = l['type']['name']
...
def load_clones(links):
    ...
    return ",".join(clones), ",".join(clone_types), len(clones), ",".join(clones_of_clones), ",".join(clone_types_of_clones)
...
def load_issues(issues):
    df = {'key': [], 'summary': [], 'description': [], 'status': [], 'status_category': [], 'created': [], \
        ...
        'sub_category_id': [], 'currentstatus_name': [], 'currentstatus_date': [], 'comments': [], 'request_type': [],\
            'priority': [], 'clones': [], 'clone_types': [], 'cloned_by': [], 'n_clones': [], 'zentrale': [], 'filiale': [], 'Link': [], 'clones_of_clones': [], 'clone_types_of_clones': []}
    for issue in issues:
        ...
        if  "issuelinks" in issue['fields'] and len(issue['fields']['issuelinks']) > 0:
            clones, clone_types, n_clones, clones_of_clones, clone_types_of_clones = load_clones(issue['fields']['issuelinks'])
    ...
    df['clone_in_project'] = df['clones'].apply(lambda x: ",".join([y.split("-")[0] for y in x.split(",")]) if x else "-")
